...
Component({
  ...
  methods: {
    ...
    drawImage(params) {
      this.ctx.save()
      const { url, top = 0, left = 0, width = 0, height = 0, borderRadius = 0, useCacheWidth = false, useCacheWidthLeft = 0 } = params
      // if (borderRadius) {
      //   this.ctx.beginPath()
      ...
      // } else {
      if (useCacheWidth) {
        console.log(this.cacheWidhtMap[useCacheWidth])
        this.ctx.drawImage(url, this.cacheWidhtMap[useCacheWidth] + useCacheWidthLeft, top, width, height)
      } else {
        this.ctx.drawImage(url, left, top, width, height)
      }
      // }
    ...
    drawText(params) {
      ...
      if (!breakWord) {
        if (cacheWidth) {
          if (!this.cacheWidhtMap) {
            this.cacheWidhtMap = {}
          }
          this.cacheWidhtMap[cacheWidth] = this.ctx.measureText(content).width;
        }
